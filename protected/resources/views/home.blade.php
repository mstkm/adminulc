...
?> -->
  ...
  <div class="btn-group" role="group">
    ...
    <a class="btn btn-default" href="{!!url('admin/nota')!!}" ><i class="glyphicon glyphicon-th-list"></i> Semua Nota</a>
</div>
<div class="col-md-5 ">
<?php if (is_null($schedule)) {?>
    <a href=""><span class="btn btn-success"> Tidak ada kegiatan dalam waktu dekat!!! </span></a>
    <?php }   
else
{
?>
<a href="{{ url('admin/schedule/'.$schedule->id)}}"><span class="btn btn-success"> <?php echo strtoupper($schedule->name).' '.date("D j M Y H:i", strtotime($schedule->start));?></span></a><?php } ?>
</div>
            <!-- Form Pencarian -->
        ...
        <table id="example1" class="table table-bordered table-striped table-hover">
            ...
            @foreach($customer as $item)
                {{-- */$x++;/* --}}
                <tr>
                    <td class="text-center">{{ $x }}</td><td><?php echo 'N'.str_pad($item->idn, 4, '000', STR_PAD_LEFT); ?></td><td><?php echo date('j M Y, H : i  ',strtotime($item->tanggal)); ?></td><td><?php echo ucwords($item->cname); ?></td><td><?php echo ucwords($item->sname) ?></td><td>
                    @if($item->test==NULL)
                    ...
                    <td class="text-center"> 
                            ...
                            <a href="{{ url('/admin/nota/' . $item->idn) }}" target="_blank" class="btn btn-warning btn-xs" title="Print Nota Customer"><span class="glyphicon glyphicon-print" aria-hidden="true"/></a>  
