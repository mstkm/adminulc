...
<div class="container">
        ...
        <h4><i class="fa fa-book"></i> Data Keuangan Kursus <?php 
        $bulan=array("Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus","September","Oktober","November","Desember");
        if ($bln>0) {
            # code... ech
            echo " Bulan ".$bln;
        } ?> Tahun <?php echo date('Y'); ?></h4><hr>
...
for($c=0; $c<$jlh_bln; $c++){
    $x=$c+1;
    echo"<li><input type=radio name=month value=0$x> $bulan[$c] </li>";
}
?>
    </ul>
  </div> 
    ...
    <ul class="dropdown-menu">
        @foreach($year as $thn)
        <li> <input type="radio" name="year" value="{{$thn->tahun}}" required> {{$thn->tahun}}</li>
        @endforeach
    </ul>
  </div>
  <button class="btn btn-primary" type="submit">Muat</button>       
            ...
            {!! Form::close() !!} 
            <div class="col-md-2 navbar-right navbar-text navbar-right">
            <a href="{{url('admin/report/finance/bulan/'.$bln.'/tahun/'.$thnpil.'/print')}}" class="btn btn-success">Unduh Laporan</a>
            </div>
            </div>
...
<div class="col-md-12">
    ...
    <div class="col-md-12">
        <div class="panel panel-default">
            ...
            <div id="chart2" >@barchart('Votes', 'chart2')</div>                       
        </div>         
    </div>
